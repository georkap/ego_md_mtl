...
def save_mt_checkpoints(model_ft, optimizer, top1, new_top1, save_all_weights, output_dir, model_name, epoch, mAP=False):
    ...
    metric_text = 'mAP_{}' if mAP else 'top1_{}'
    save_dict = dict()
    save_dict['epoch'] = epoch
    save_dict['state_dict'] = model_ft.state_dict()
    save_dict['optimizer'] = optimizer.state_dict()
    for ind in range(len(new_top1)):
        save_dict[metric_text.format(ind)] = new_top1[ind]

    torch.save(save_dict, weight_file)

    cur_top1s = list()
    for ind in range(len(new_top1)):
        addon = "_task{}".format(ind)
        if mAP:
            addon += "_mAP"
        tasktop1 = save_best_checkpoint(top1[ind], new_top1[ind], output_dir, model_name + addon, weight_file)
