...
class MFNET_3D_TDN(nn.Module):
    def __init__(self, num_classes, dropout=None, **kwargs):
        super(MFNET_3D_TDN, self).__init__()
        ...
        self.tail_tdn = nn.Sequential(OrderedDict([tailnorm, ('relu', nn.ReLU(inplace=True))]))
        ...
        self.classifier_list_tdn = MultitaskClassifiers(c5_out, num_classes)
...
if __name__ == "__main__":
    ...
    kwargs = {'num_coords': 3, 'num_objects': None, 'num_obj_cat': None, 'one_object_layer': True,
              'ensemble_eval': False}
    net = MFNET_3D_TDN(num_classes=[106, 19, 53], dropout=0.5, **kwargs)
